...
async fn test_create_server() {
    ...
    let (token, _user_id) = app
        .register_user(&unique_name("cs"), &unique_email("cs"), "TestPass123!")
        .await;
...
async fn test_create_server_member_count() {
    let app = get_test_app().await;
    let (token, _uid) = app
        .register_user(&unique_name("mc"), &unique_email("mc"), "TestPass123!")
        .await;
...
async fn test_create_server_has_general_channel() {
    ...
    let general = channels
        .iter()
        .find(|c| c["type"].as_i64() == Some(0) && c["name"].as_str() == Some("general"));
    assert!(
        general.is_some(),
        "Server should have a default 'general' text channel"
    );
}
...
async fn test_create_server_has_voice_channel() {
    ...
    assert!(
        voice.is_some(),
        "Server should have a default voice channel"
    );
}
...
async fn test_get_server() {
    ...
    let res = app
        .get(&token, &format!("/api/v1/servers/{}", server_id))
        .await;
    assert_eq!(res.status(), 200);
...
async fn test_get_server_not_member() {
    ...
    let (other_token, _) = app
        .register_user(
            &unique_name("outsider"),
    ...
    let res = app
        .get(&other_token, &format!("/api/v1/servers/{}", server_id))
        .await;
...
async fn test_server_limit_100() {
    ...
    let (token, user_id) = app
        .register_user(&unique_name("sl"), &unique_email("sl"), "TestPass123!")
        .await;
    ...
    for i in 0..100 {
        ...
        sqlx::query(
            "INSERT INTO servers (id, name, owner_id, member_count) VALUES ($1, $2, $3, 1)",
        )
        .bind(sid)
...
async fn test_use_invite() {
    ...
    let (new_token, _new_uid) = app
        .register_user(
            &unique_name("invitee"),
            &unique_email("invitee"),
            "TestPass123!",
        )
        .await;
...
async fn test_use_invite_max_uses() {
    ...
    // First user joins
    let (t1, _) = app
        .register_user(&unique_name("mu1"), &unique_email("mu1"), "TestPass123!")
        .await;
    let res1 = app
        .post(
            &t1,
    ...
    let (t2, _) = app
        .register_user(&unique_name("mu2"), &unique_email("mu2"), "TestPass123!")
        .await;
    let res2 = app
        .post(
            &t2,
...
async fn test_use_invite_expired() {
    ...
    let (t, _) = app
        .register_user(&unique_name("exp"), &unique_email("exp"), "TestPass123!")
        .await;
    let res = app
        .post(
            &t,
...
async fn test_list_server_invites() {
    ...
    let res = app
        .get(&token, &format!("/api/v1/servers/{}/invites", server_id))
        .await;
    assert_eq!(res.status(), 200);
...
async fn test_delete_invite() {
    ...
    let res = app
        .delete(&token, &format!("/api/v1/invites/{}", code))
        .await;
    assert_eq!(res.status(), 204);
...
async fn test_delete_invite_by_creator() {
    ...
    let res = app
        .delete(&member_token, &format!("/api/v1/invites/{}", code))
        .await;
    assert_eq!(res.status(), 204);
}
...
async fn test_list_members() {
    ...
    let res = app
        .get(
            &owner_token,
            &format!("/api/v1/servers/{}/members", server_id),
        )
        .await;
    assert_eq!(res.status(), 200);
...
async fn test_list_members_pagination() {
    ...
    let res = app
        .get(
            &owner_token,
            &format!("/api/v1/servers/{}/members?limit=1", server_id),
        )
        .await;
    ...
    let res2 = app
        .get(
            &owner_token,
            &format!(
                "/api/v1/servers/{}/members?limit=1&after={}",
                server_id, first_id
            ),
        )
        .await;
...
async fn test_get_member() {
    ...
    let body: serde_json::Value = res.json().await.unwrap();
    assert_eq!(
        body["user"]["id"].as_str().unwrap().parse::<i64>().unwrap(),
        member_id
    );
}
...
async fn test_kick_member_not_permitted() {
    ...
    let res = app
        .delete(
            &member_token,
            &format!("/api/v1/servers/{}/members/{}", server_id, target_id),
        )
        .await;
    assert_eq!(res.status(), 403);
}
...
async fn test_kick_owner() {
    ...
    let res = app
        .delete(
            &owner_token,
            &format!("/api/v1/servers/{}/members/{}", server_id, owner_id),
        )
        .await;
    assert_eq!(res.status(), 403);
}
...
async fn test_leave_server() {
    ...
    let res = app
        .delete(
            &member_token,
            &format!("/api/v1/servers/{}/leave", server_id),
        )
        .await;
    assert_eq!(res.status(), 204);
}
...
async fn test_transfer_ownership() {
    ...
    let body: serde_json::Value = res.json().await.unwrap();
    assert_eq!(
        body["owner_id"].as_str().unwrap().parse::<i64>().unwrap(),
        new_owner_id
    );
...
async fn test_role_hierarchy_kick() {
    ...
    app.patch(
        &owner_token,
        &format!("/api/v1/servers/{}/members/{}", server_id, high_uid),
        serde_json::json!({
    ...
    app.patch(
        &owner_token,
        &format!("/api/v1/servers/{}/members/{}", server_id, low_uid),
        serde_json::json!({
...
async fn test_role_hierarchy_ban() {
    ...
    app.patch(
        &owner_token,
        &format!("/api/v1/servers/{}/members/{}", server_id, mod_uid),
        serde_json::json!({
    ...
    app.patch(
        &owner_token,
        &format!("/api/v1/servers/{}/members/{}", server_id, target_uid),
        serde_json::json!({
            "roles": [high_role_id, everyone_role_id(server_id)]
        }),
    )
    ...
    let res = app
        .put(
            &mod_token,
            &format!("/api/v1/servers/{}/bans/{}", server_id, target_uid),
            serde_json::json!({
                "reason": "test"
            }),
        )
...
async fn test_create_ban() {
    ...
    let banned_member = members
        .iter()
        .find(|m| m["user"]["id"].as_str().unwrap().parse::<i64>().unwrap() == member_id);
    assert!(
        banned_member.is_none(),
        "Banned member should be removed from member list"
    );
}
...
async fn test_create_ban_not_permitted() {
    ...
    let res = app
        .put(
            &member_token,
            &format!("/api/v1/servers/{}/bans/{}", server_id, target_id),
            serde_json::json!({
                "reason": "Nope"
            }),
        )
...
async fn test_list_bans() {
    ...
    app.put(
        &owner_token,
        &format!("/api/v1/servers/{}/bans/{}", server_id, uid1),
        serde_json::json!({
            "reason": "ban1"
        }),
    )
    ...
    app.put(
        &owner_token,
        &format!("/api/v1/servers/{}/bans/{}", server_id, uid2),
        serde_json::json!({
            "reason": "ban2"
        }),
    )
    ...
    let res = app
        .get(&owner_token, &format!("/api/v1/servers/{}/bans", server_id))
        .await;
    assert_eq!(res.status(), 200);
...
async fn test_get_ban() {
    ...
    app.put(
        &owner_token,
        &format!("/api/v1/servers/{}/bans/{}", server_id, uid),
        serde_json::json!({
            "reason": "naughty"
        }),
    )
...
async fn test_ban_owner() {
    ...
    let res = app
        .put(
            &owner_token,
            &format!("/api/v1/servers/{}/bans/{}", server_id, owner_id),
            serde_json::json!({
                "reason": "self-ban"
            }),
        )
...
async fn test_ban_removes_membership() {
    ...
    let before = app
        .get(
            &owner_token,
            &format!("/api/v1/servers/{}/members/{}", server_id, uid),
        )
        .await;
...
async fn test_create_channel_in_category() {
    ...
    let body: serde_json::Value = res.json().await.unwrap();
    assert_eq!(
        body["parent_id"].as_str().unwrap().parse::<i64>().unwrap(),
        cat_id
    );
}
...
async fn test_create_channel_max_500() {
    ...
    for i in 0..498 {
        ...
        sqlx::query(
            "INSERT INTO channels (id, server_id, type, name, position) VALUES ($1, $2, 0, $3, $4)",
        )
        .bind(cid)
        .bind(server_id)
        .bind(format!("ch-{}", i))
...
async fn test_get_channel() {
    ...
    let res = app
        .get(&token, &format!("/api/v1/channels/{}", channel_id))
        .await;
    assert_eq!(res.status(), 200);
...
async fn test_delete_channel() {
    ...
    let res = app
        .delete(&token, &format!("/api/v1/channels/{}", ch_id))
        .await;
    assert_eq!(res.status(), 204);
}
...
async fn test_delete_last_text_channel() {
    ...
    let res = app
        .delete(&token, &format!("/api/v1/channels/{}", channel_id))
        .await;
    assert_eq!(res.status(), 400);
}
...
async fn test_delete_category_moves_children() {
    ...
    // Delete category
    let del_res = app
        .delete(&token, &format!("/api/v1/channels/{}", cat_id))
        .await;
    assert_eq!(del_res.status(), 204);

    // Child should now have no parent
    let child_after = app
        .get(&token, &format!("/api/v1/channels/{}", child_id))
        .await;
    let body: serde_json::Value = child_after.json().await.unwrap();
    assert!(
        body["parent_id"].is_null(),
        "Child should become parentless after category deletion"
    );
}
...
async fn test_reorder_channels() {
    ...
    let first_text = channels
        .iter()
        .find(|c| c["type"].as_i64() == Some(0) && c["position"].as_i64() == Some(0));
    assert!(first_text.is_some());
    assert_eq!(
        first_text.unwrap()["id"]
            .as_str()
            .unwrap()
            .parse::<i64>()
            .unwrap(),
...
async fn test_create_role() {
    ...
    assert!(!body["is_default"].as_bool().unwrap());
}
...
async fn test_list_roles() {
    ...
    let res = app
        .get(&token, &format!("/api/v1/servers/{}/roles", server_id))
        .await;
    assert_eq!(res.status(), 200);
...
async fn test_role_hierarchy_prevents_editing_higher() {
    ...
    let res = app
        .patch(
            &member_token,
            &format!("/api/v1/servers/{}/roles/{}", server_id, high_role_id),
            serde_json::json!({
                "name": "Hijacked"
            }),
        )
...
async fn test_reorder_roles() {
    ...
    let role_a = roles
        .iter()
        .find(|r| r["id"].as_str().unwrap().parse::<i64>().unwrap() == r1_id)
        .unwrap();
...
async fn test_set_channel_override() {
    ...
    let res = app
        .put(
            &token,
            &format!("/api/v1/channels/{}/permissions/{}", channel_id, server_id),
            serde_json::json!({
                "type": 0,
                "allow": "16384",
                "deny": "0"
...
async fn test_channel_override_strips_admin() {
    ...
    if !overrides.is_empty() {
        ...
        assert_eq!(
            allow & 1,
            0,
            "ADMINISTRATOR bit should be stripped from channel overrides"
        );
    }
}
...
async fn test_delete_channel_override() {
    ...
    let found = overrides
        .iter()
        .find(|o| o["id"].as_str().unwrap().parse::<i64>().unwrap() == server_id);
